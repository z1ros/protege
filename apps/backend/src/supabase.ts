...
/* ==========================================================
   GitHub id → Supabase UUID resolver.

   Every child table keys on `user_id UUID REFERENCES users(id)`, but the
   backend's per-request `userId` is whatever the extension sends — which
   after auth is the GitHub numeric id, not the Supabase row's autogenerated
   UUID. Passing that straight to `.eq("user_id", userId)` produces
   `invalid input syntax for type uuid`; passing it into an upsert produces
   `foreign key constraint … violates users_id_fkey` once the child table
   tries to reference a row that doesn't exist yet.

   This resolver sits in front of every cloud helper. It accepts the
   request-scope userId, looks up (or creates) the matching users row by
   github_id, and returns the Supabase UUID. Results are memoized for the
   process lifetime (github_id → uuid is immutable). Concurrent resolves
   for the same github_id are coalesced so a cold start doesn't fan out
   into N duplicate upserts.
   ========================================================== */

const UUID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const githubIdToCloudUuid = new Map<string, string>();
const cloudUserResolves = new Map<string, Promise<string | null>>();
...
/**
 * Map a request-scope userId to the Supabase `users.id` UUID used by every
 * FK-bearing child table. Pass-through for values that are already UUIDs
 * (legacy local-dev callers, cold-sync code that already resolved).
 *
 * Returns null when Supabase isn't configured or the lookup/insert failed;
 * callers must skip the cloud write in that case.
 */
export async function resolveCloudUserUuid(
  userId: string
): Promise<string | null> {
  if (!userId) return null;

  // Pass-through for UUID-shaped inputs, but verify the row still exists.
  // Local store may carry stray UUIDs from old seed runs or a wiped Supabase
  // project; writing those straight to a child FK column trips
  // `<table>_user_id_fkey`. Probe + cache so we don't pay a round-trip per
  // shadow write.
  if (looksLikeUuid(userId)) {
    if (githubIdToCloudUuid.get(userId) === userId) return userId;
    const inFlightUuid = cloudUserResolves.get(userId);
    if (inFlightUuid) return inFlightUuid;
    const probe: Promise<string | null> = (async () => {
      ...
      const { data } = await sb
        ...
        .maybeSingle();
      if (data?.id) {
        githubIdToCloudUuid.set(userId, data.id);
        return data.id;
      }
      return null;
    })();
    cloudUserResolves.set(userId, probe);
    try {
      return await probe;
    } finally {
      cloudUserResolves.delete(userId);
    }
  }

  const cached = githubIdToCloudUuid.get(userId);
  if (cached) return cached;

  const inFlight = cloudUserResolves.get(userId);
  if (inFlight) return inFlight;

  const p: Promise<string | null> = (async () => {
    const sb = getSupabase();
    if (!sb) return null;
    // Probe first so we don't overwrite login/email on a returning user.
    const existing = await sb
      ...
      .maybeSingle();
    if (existing.data?.id) {
      githubIdToCloudUuid.set(userId, existing.data.id);
      return existing.data.id;
    }
    // First-time user. login is required by the column; we don't have the
    // handle here, so seed with the github id as placeholder. The real login
    // lands the next time the auth middleware resolves a token for this
    // user via `ensureCloudUser`.
    const { data, error } = await sb
      .from("users")
      .upsert(
        { github_id: userId, login: userId },
    ...
    if (error || !data?.id) {
      console.warn(
        "[protege] resolveCloudUserUuid failed:",
  ...
  cloudUserResolves.set(userId, p);
  try {
    return await p;
  } finally {
    cloudUserResolves.delete(userId);
...
/** Test-only: clear the memoized github→uuid mappings. */
export function _resetCloudUserCacheForTests(): void {
  githubIdToCloudUuid.clear();
  cloudUserResolves.clear();
}
...
export async function ensureCloudUser(
  ...
  // Warm the resolver cache so subsequent cloud helpers skip the probe.
  githubIdToCloudUuid.set(githubId, data.id);
  return data.id;
}
...
export async function recordCloudConcepts(
  ...
  const sb = getSupabase();
  if (!sb) return;
  const supa = await resolveCloudUserUuid(userId);
  if (!supa) return;
...
export async function getUserPercentile(
  ...
  const supa = await resolveCloudUserUuid(userId);
  if (!supa) return { rank: 0, total: 0, percentile: 0 };
...
export interface EchoEventCloudInput {
  /** Stable client-side dedup id. Required: the upsert relies on
   *  (user_id, client_event_id) as its conflict key. */
...
export async function cloudAppendEchoEvents(
  ...
  // Idempotent upsert keyed on the unique index from migration 001.
  // ignoreDuplicates: a re-sent event collides with the existing row and
  // is silently skipped — matches the local store's same-id dedup.
...
export async function cloudReadEchoEvents(
  ...
  let query = sb
    .from("echo_events")
    .select("client_event_id, event_type, ts, file, payload")
    .eq("user_id", supa)
    .gte("ts", sinceMs)
...
export async function cloudUpsertBehaviorRollup(
  ...
  const sb = getSupabase();
  if (!sb) return;
  const supa = await resolveCloudUserUuid(row.userId);
  if (!supa) return;
...
export async function cloudAppendConceptEncounter(
  ...
  // Day-grain dedup matches the local store's appendConceptEncounter
  // (which already drops same-day duplicates). The unique index in
  // migration 001 covers (user_id, concept, file_path, seen_at_day);
  // seen_at_day is a STORED generated column derived from seen_at, so
  // we send seen_at and Postgres fills the dedup key automatically.
  const { error } = await sb
    ...
    .upsert(
      ...
      {
        onConflict: "user_id,concept,file_path,seen_at_day",
        ignoreDuplicates: true,
      }
    );
  if (error) {
...
/**
 * Atomic bump: delegates to the `bump_file_authorship` Postgres RPC
 * (migration 001) so concurrent calls for the same (userId, filePath)
 * each add their delta to the row in a single statement, eliminating
 * the read-then-upsert lost-update race that systematically undercounted
 * W14 independence metrics.
 *
 * Negative or non-finite deltas are clamped to 0 before the round-trip;
 * the RPC also clamps server-side as a defence in depth.
 */
export async function cloudBumpFileAuthorship(
  ...
  const { error } = await sb.rpc("bump_file_authorship", {
    uid: supa,
    fp: filePath,
    hd: h,
    ad: a,
  ...
  if (error) {
    console.warn("[protege] cloudBumpFileAuthorship rpc failed:", error.message);
  }
}
