...
import { hasNativeDiagnosticInRange } from "../review/nativeDiagnostics.js";
import { shouldSuppress as gateShouldSuppress, onGateChanged } from "../review/findingGate.js";
  ...
  disposables.push(
    vscode.commands.registerCommand(
      ...
      async (args: { uri: string; line: number }) => {
        ...
        const { resolveExplainMode } = await import("../teaching/explainMode.js");
  ...
  disposables.push(
    vscode.commands.registerCommand(
      ...
      async (args: { uri: string; line: number }) => {
        ...
        const { dismissSuggestionAtLine } = await import("../review/liveReview.js");
...
export function buildActionHover(
  s: Suggestion,
  uri: string,
  lang: string
  ...
  // verbs — so users learn one vocabulary that works everywhere.
  // Dingbats ✔ ✿ ✘ are the Protege brand marks for Apply / Teach /
  // Dismiss. They render as text glyphs in VS Code's editor font (no
  // emoji-style colored fallback) and stay consistent across CodeLens,
  // MarkdownString, and QuickPick surfaces.
  const parts: string[] = [];
