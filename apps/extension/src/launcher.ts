    ...
    view.webview.onDidReceiveMessage(async (msg: { type: string }) => {
      ...
      if (msg.type === "open") {
        ...
        try {
          ...
          // Don't read panel.viewType here — accessing the viewType
          // getter on a disposed panel throws "Webview is disposed"
          // synchronously, which would surface as a spurious error to
          // the user even when openProtegePanel actually succeeded
          // earlier in this turn. Just log existence + column.
          log(
            "launcher.host",
            `openProtegePanel resolved — panel=${panel ? "present" : "undefined"}`
          );
        } catch (err) {
          log(
            "launcher.host",
