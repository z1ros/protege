...
async function checkExerciseInline(
  ...
  if (hasTodo) {
    // Still has blanks — show encouragement
    const todoLine = findLine(editor.document, "???") ?? findLine(editor.document, "TODO");
    if (todoLine !== null) {
      editor.setDecorations(decoration, [{
        range: new vscode.Range(todoLine, 0, todoLine, 0),
        renderOptions: {
          after: {
            contentText: "  ← fill this in",
