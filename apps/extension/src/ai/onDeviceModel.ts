 ...
 *
 * `storagePath` MUST be a persistent writable directory — typically
 * `context.globalStorageUri.fsPath`. It must NOT be `context.extensionPath`:
 * the extension install directory is version-numbered (e.g.
 * `~/.cursor/extensions/protege-protege-0.0.1/`), is meant to be
 * read-only, and gets replaced by a new dir on every extension update.
 * Writing the 4.7 GB model there orphans it on every rebuild and forces
 * a fresh ~5-minute download — exactly the "it re-downloads every time"
 * bug users hit before this fix.
 */
export async function initOnDeviceModel(
  ...
  try {
    ...
    // Persistent cache dir inside globalStorage — survives extension
    // updates because VS Code/Cursor route globalStorageUri to a stable
    // per-extension location (e.g. `~/Library/Application Support/
    // Cursor/User/globalStorage/protege.protege/`).
    const cacheDir = path.join(storagePath, "model-cache");
    if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir, { recursive: true });

    const modelPath = path.join(cacheDir, MODEL_FILE);

    // One-time migration: if the user downloaded the model under the
    // old extensionPath-based cache before this fix, reuse it instead
    // of forcing a fresh 4.7 GB download. We check a couple of common
    // Cursor/VS Code paths — any hit gets moved into the new location.
    if (!fs.existsSync(modelPath)) {
      ...
      for (const root of legacyCandidates) {
        if (!fs.existsSync(root)) continue;
        try {
          const dirs = fs.readdirSync(root);
          const ours = dirs.filter((d) => d.startsWith("protege"));
          for (const d of ours) {
          ...
          // One legacy root being unreadable shouldn't block the rest.
        }
      }
    }
