...
import { broadcast, mountedWebviewCount } from "./webviewHost.js";
import { setWakeSuspended } from "./voiceCapture.js";
import type { HighlightRegion } from "@protege/types";
...
export async function runTeachStep(args: TeachStepArgs): Promise<string> {
  ...
  try {
    ...
    if (h && h.path && Number.isFinite(h.startLine) && Number.isFinite(h.endLine)) {
      try {
        const { highlightCodeForTeaching } = await import("./tools.js");
